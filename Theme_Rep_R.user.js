...
// @noframes
// @grant        none
// @updateURL        https://github.com/personwritep/Theme_Rep_R/raw/main/Theme_Rep_R.user.js
// @downloadURL        https://github.com/personwritep/Theme_Rep_R/raw/main/Theme_Rep_R.user.js
// ==/UserScript==
...
function storage(){
    ...
    if(frame){ // 旧レトロスキン ブログ画面で動作
        theme_list=[[ , ]]; // リセット

        let btn_arch=document.querySelectorAll('#theme_list li a');
...
function main(){
    if(document.querySelector('#frame')){ // レトロスキンブログ画面で動作
        ...
        if(help_TR){
            let url='https://ameblo.jp/personwritep/entry-12797556818.html';
        ...
        let style=
            ...
            '.menu_frame li a[aria-current="page"] { '+
            'color: red; text-shadow: 0.5px 0 red; } '+
            '#recent_entries_list li { padding: 6px 4px 6px 6px; background: #fff; } '+
            '#recent_entries_list li a, #recent_entries_list li a:hover, '+
            '#recent_entries_list li a:focus { color: #000; } '+
            '#recent_entries_list .newEntryUserAction * { color: #888; } '+
            '#recent_entries_list li:hover { '+
            'outline: 1px solid #2196f3; outline-offset: -1px; '+
            'box-shadow: inset 0 0 0 60px rgba(187, 222, 251, .3); } '+
            '#recent_entries_list li:hover a { '+
        ...
        if(themes_get){
            themes_get.onclick=function(){
                let tab_active=document.querySelector('#recent_entries_list');
                if(tab_active){ // 記事リストを開いている場合
                    reload_act(); }
                else{ // 記事リストを開いていない場合
                    let first_theme=document.querySelector('#theme_list li a');
                    if(first_theme){
                        let theme_url=first_theme.getAttribute('href');
                        location.href=theme_url; } // 最初のテーマの記事リストを開く
                }}}
    ...
    if(tab_active){ // 記事リスト画面で動作
        ...
        function select_set(){
            let ac_list=document.querySelectorAll('#recent_entries_list li');
        ...
        function select_array(new_theme){
            ...
            for(let k=0; k<ac_list.length; k++){
                if(ac_list[k].classList.contains('select_item')){
                    ...
                    let title_link=ac_list[k].querySelector('.contentTitle');
